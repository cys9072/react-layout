...
<html>
  <head>
    <meta charset="UTF-8">
    <title>마커 클러스터 기준 거리 설정하기</title>
    <style>
      #map {
        width: 100%;
        height: 500px;
  ...
  <body>
    <div id="map"></div>
    <label htmlFor="gridSize">기준 거리: </label>
    <input id="gridSize" type="number" value="80">
    <script>
    function initMap() {
      ...
      document.querySelector("#gridSize").addEventListener("change", function(e) {
        markerCluster.setGridSize(+e.target.value);
      });
    }  
    </script>
...
const MarkerClusterSettingDistancePage = () => {
  return (
    <div className="container">
      <MapTitle before="마커" label="마커 클러스터 기준 거리 설정하기" />
      <div className="contents no-border">
        <MapDesc
          label={
            <>
              지도 상의 특정 지점을 클릭하면 해당 위치에 마커를 추가하고 해당 범위의 클러스터 정보를 업데이트 합니다.
              <br />
              기준 거리 값의 설정에 따라 클러스터 생성에 대한 기준 범위가 달라집니다.
              <br />
              기준 거리 값이 커지면 클러스터 생성 범위가 넓어지고, 기준 거리 값이 작아지면 클러스터 생성 범위가 좁아집니다.
              <br />
              하단의 창에 입력된 기준 거리를 변경해보세요.
            </>
          }
        />
        ...
        <div className="options">
          <label htmlFor="gridSize">기준 거리: </label>
          <input id="gridSize" type="number" defaultValue="80" />
        </div>
...
export default MarkerClusterSettingDistancePage;
